...
function eliminarPeriodo(p) {
    if (confirm(`¿Eliminar el período vacío "${p.descripcion}" de ${p.empresa}?`)) {
        router.delete(route('planilla.periodos.destroy', p.id), { preserveScroll: true });
    }
}
                            ...
                            <tr v-for="p in periodosVisibles" :key="p.id" class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <button v-if="puedeGenerar && !p.payroll" @click="eliminarPeriodo(p)" class="inline-flex items-center gap-1 rounded-md bg-red-50 px-3 py-1.5 text-xs font-semibold text-red-700 hover:bg-red-100">🗑 Eliminar</button>
        ...
        <div v-if="avisoAsistencia" class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 p-4" @click.self="avisoAsistencia = ''">
            <div class="w-full max-w-2xl rounded-xl bg-white shadow-2xl">
                <div class="flex items-center justify-between border-b px-5 py-4">
                    <h3 class="text-lg font-bold text-red-700">⚠️ No se puede procesar la planilla</h3>
                ...
                <div class="flex justify-end border-t px-5 py-4">
                    <button @click="avisoAsistencia = ''" class="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white">Entendido</button>
                </div>
            </div>
        </div>
    </AuthenticatedLayout>
