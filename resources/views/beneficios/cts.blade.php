...
<style>
    * { font-family: DejaVu Sans, sans-serif; }
    ...
    table.det th { background: #0E7490; color: #fff; padding: 5px 6px; text-align: left; font-size: 9.5px; }
    table.det td { padding: 5px 6px; border-bottom: 1px solid #ddd; }
    .num { text-align: right; }
    .tot { background: #ecfeff; font-weight: bold; }
...
@php
    ...
    $periodoTxt = ($c->periodo === 'noviembre' ? 'Noviembre (semestre may–oct)' : 'Mayo (semestre nov–abr)').' '.$c->anio;
@endphp

<div class="emp">{{ $empresa->razon_social }}</div>
<div class="muted">RUC {{ $empresa->ruc }}@if($empresa->direccion) · {{ $empresa->direccion }}@endif</div>

<h1>LIQUIDACIÓN DE CTS</h1>
<div class="sub">Compensación por Tiempo de Servicios — Depósito de {{ $periodoTxt }}</div>
...
<table class="datos">
    ...
    <tr>
        <td><b>Depósito</b></td><td>{{ $periodoTxt }}</td>
        <td><b>Tiempo</b></td><td>{{ $c->meses_computables }} meses{{ $c->dias_computables ? ' y '.$c->dias_computables.' días' : '' }}</td>
    </tr>
</table>
...
<table class="det">
    ...
    <tbody>
        <tr><td>Remuneración + asignación familiar</td><td class="num">{{ $money($c->rem_computable - $c->sexto_gratificacion) }}</td></tr>
        <tr><td>1/6 de gratificación</td><td class="num">{{ $money($c->sexto_gratificacion) }}</td></tr>
        <tr><td>Remuneración computable</td><td class="num">{{ $money($c->rem_computable) }}</td></tr>
        <tr><td>CTS ({{ $c->meses_computables }}/12 + {{ $c->dias_computables }}/360)</td><td class="num">{{ $money($c->monto) }}</td></tr>
        <tr class="tot"><td>TOTAL A DEPOSITAR</td><td class="num">{{ $money($c->monto) }}</td></tr>
    </tbody>
</table>

<p class="muted" style="margin-top:14px;">Monto a depositar en la cuenta CTS del trabajador dentro de los primeros 15 días del mes, conforme al D.S. 001-97-TR.</p>
